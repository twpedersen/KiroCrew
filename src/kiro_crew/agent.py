...
def _claimed_app_agent_filenames(apps: list[Any]) -> set[str]:
    """Materialized ``<app>--*.json`` names owned by a listed app."""
    claimed: set[str] = set()
    for info in apps:
        ...
        if not isinstance(name, str) or not name:
            continue
        claimed.update(_remaining_app_agent_names(name))
    return claimed


def _orphan_app_agent_filenames(claimed: set[str]) -> list[str]:
    """``<app>--*.json`` files no listed app claims (host agents have no ``--``)."""
    from kiro_crew.apps.bridges import _kiro_agents_dir
    ...
    orphans: list[str] = []
    for entry in agents_dir.iterdir():
        if not entry.name.endswith(".json") or "--" not in entry.name:
            continue
...
def _neutralize_unrefreshed_app_agents() -> None:
    """Empty leftover MCP on ``<app>--*.json`` after a failed login refresh.
    ...
    loses its command. Materialized ``<app>--*.json`` files that no
    listed app claims (missing or unreadable ``installed.json``) are
    neutralized the same way. Apps with ``resources="app"`` manage
    their own agents and are skipped.
    """
    try:
    ...
    for filename in _orphan_app_agent_filenames(_claimed_app_agent_filenames(apps)):
        _neutralize_app_agent_file(filename)


def _refresh_enabled_app_agents(*, fail_closed: bool = False) -> None:
    ...
    unlink cannot leave a withheld command on disk. Fail-closed
    login also empties materialized ``<app>--*.json`` files that no
    listed app claims, so a missing ``installed.json`` cannot leave
    a withheld command on disk. Apps with ``resources="app"``
    manage their own agents and are skipped so a no-op refresh
    cannot prune or neutralize those files. Gateway boot skips
    this walk (``rebuild_agent_config(app_agent_refresh=False)``).
    """
    failures: list[str] = []
    try:
    ...
    for info in apps:
        ...
        if fail_closed:
            ...
            failures.extend(f"{name}: leftover {fn}" for fn in leftover)
    if fail_closed:
        claimed = _claimed_app_agent_filenames(apps)
        for filename in _orphan_app_agent_filenames(claimed):
            if not _neutralize_app_agent_file(filename):
                failures.append(f"orphan leftover {filename}")
...
def _restore_authored_mcp(
    ...
    not unlink: the sidecar stays until every restored name survives
    command validation and the runtime spec is durably written.
    *restored_names*, when supplied, receives those kept names so the
    writer can refuse to unlink a stash whose only copy just failed
    validation.
    """
    path = _authored_mcp_path()
    try:
...
def rebuild_agent_config(*, clean: bool = False, app_agent_refresh: bool = True) -> Path:
    ...
    if app_agent_refresh:
        if login_withhold:
            ...
            try:
                _refresh_enabled_app_agents(fail_closed=True)
            except Exception:
