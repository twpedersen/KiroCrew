...
export const api = {
  ...
  // Task runner
  projectBundles: () => fetch('/api/projects').then(j) as Promise<ProjectBundlesResponse>,
  createProjectBundle: (name: string, path: string) =>
    post('/api/projects', { name, path }).then(j) as Promise<ProjectBundle>,
  addProjectBundle: (source: string) =>
    post('/api/projects/add', { source }).then(j) as Promise<ProjectBundle>,
  updateProjectBundle: (id: string, body: import('../types').ProjectBundleUpdate) =>
    patch('/api/projects/' + encodeURIComponent(id), body).then(j) as Promise<ProjectBundle>,
  syncProjectBundle: (id: string) =>
    post('/api/projects/' + encodeURIComponent(id) + '/sync', {}).then(j) as Promise<ProjectBundle>,
  activateProjectBundle: (id: string, expectedKey: string) =>
    post('/api/projects/' + encodeURIComponent(id) + '/activate', { expected_key: expectedKey }).then(j) as Promise<ProjectBundle['capabilities']>,
  deactivateProjectBundle: (id: string) =>
    del('/api/projects/' + encodeURIComponent(id) + '/activate').then(j) as Promise<ProjectBundle['capabilities']>,
  removeProjectBundle: (id: string) =>
    del('/api/projects/' + encodeURIComponent(id)).then(j) as Promise<{ ok: true; id: string }>,
