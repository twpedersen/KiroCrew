...
def test_slot_queue_stamps_principal_only_when_both_fields_are_set() -> None:
    from kiro_crew.dashboard.state import _ChatSlot

    slot = _ChatSlot("principal-queue")
    stamped_id = slot.queue_append(
        "human",
        principal_surface="dashboard",
        principal_raw_id="alice",
    )
    partial_id = slot.queue_append(
        "partial",
        principal_surface="dashboard",
        principal_raw_id="",
    )
    inserted_id = slot.queue_insert(
        0,
        "inserted",
    ...
    assert by_id[stamped_id]["_principal_surface"] == "dashboard"
    assert by_id[stamped_id]["_principal_raw_id"] == "alice"
    assert "_principal_surface" not in by_id[partial_id]
    assert by_id[inserted_id]["_principal_surface"] == "slack"
    assert by_id[inserted_id]["_principal_raw_id"] == "U1"

    assert slot.queue_edit_by_id(
        stamped_id,
        "replaced",
        principal_surface="dashboard",
        principal_raw_id="bob",
    )
    assert by_id[stamped_id]["_principal_raw_id"] == "bob"
    assert slot.queue_edit_by_id(stamped_id, "cleared")
    assert "_principal_surface" not in by_id[stamped_id]
    assert "_principal_raw_id" not in by_id[stamped_id]


@pytest.mark.asyncio
async def test_prepare_turn_gateway_unbinds_cross_namespace_bind(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from kiro_crew.messaging.identity import prepare_turn_gateway
    ...
    async def _fake_prepare(
        ...
        *,
        ...
        del session_key, sessions, agent
        seen.append((surface, raw_id))

    monkeypatch.setattr(
        "kiro_crew.platform.agentcore_gateway.prepare_session_gateway",
        _fake_prepare,
    )
    await prepare_turn_gateway(
        object(),
        "dashboard:1",
        {"surface": "slack", "raw_id": "U0123"},
    )
    await prepare_turn_gateway(
        object(),
        "slack:1783733803.877979",
        {"surface": "slack", "raw_id": "U0123"},
    )
    assert seen == [(None, None), ("slack", "U0123")]
