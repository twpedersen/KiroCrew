...
| `kirocrew.session.duration` | histogram (ms) | `end_reason` (`reset` / `removed` / `unclaimed` / `destroyed` / `discarded` / `shutdown` / `retired` / `recycled` / `crashed`), `session_source` (via `messaging.link::telemetry_channel_of`) | `metrics/sessions.py::record_session_ended` for every clean end; `metrics/sessions.py::backfill_crashed_sessions` for `crashed`. **Why the reason is passed in, never derived.** There is no single teardown funnel: `session_lifecycle.py` holds six end paths that each pop the registry themselves (`reset` / `remove` / `remove_if_unclaimed` / `destroy` / `discard_conversation` / `close_all`), and two more pop it outside all of those — `retire_kiro_identity_sessions` reports `retired` (an identity change tearing down idle processes on the old account, from both its stale-clear and its doomed-session branch), and `session_compaction.py::CompactionCoordinator._recycle_held` reports `recycled` (context overflow replacing the provider in place). So each site states its own `end_reason`, the same contract `metrics/turns.py` holds its callers to. Every path calls this in the SAME event-loop tick as its `_sessions.pop`, before its first `await`: the call is non-blocking (an in-memory pop, an in-memory histogram record, one `unlink`), and a later call lets a racing cold start register a SUCCESSOR under the reused key whose record the predecessor's teardown then consumes. `reset` is a PATH label rather than a cause — the idle sweep and a slot reset both reach teardown through it, and the finer causes are already counted by `kirocrew.session.idle_expired` and `kirocrew.watchdog.recovery.outcome`; `retired` and `recycled` are separate precisely because they never take the `reset` route. **The crumb is what makes a crash countable.** A start time held only in memory dies with the process, which is exactly the population most worth measuring, because a crash runs no teardown path at all. So `record_session_started` drops one small JSON file under `<data home>/metrics/open-sessions/`, named by SHA-256 digest of the session key with the key stored inside (keys carry channel ids and slot names and are not filesystem-safe; `session_map.json` already holds every key in the same data home, so this is not new exposure). A clean end unlinks it; whatever survives to the next boot is a session that never ended cleanly, which `backfill_crashed_sessions(started_before)` emits as `crashed` and unlinks. `started_before` is the calling process's own start time, so a crumb at or after it is left alone — that cutoff is what lets the scan run OFF the boot path without mistaking a session THIS run just opened for a casualty of the last. **Exactly-once accounting.** A crumb is consumed at most once, so the teardown paths — which are NOT mutually exclusive, the idle sweep calls `reset` — cannot double-count between them, and a backfilled session cannot be counted again on the boot after. The clean path deliberately reads neither the crumb nor the popped entry's `created_at`: consuming a DIFFERENT record than the crash path would let a clean end's leftover crumb be re-emitted as `crashed`, and `created_at` is reset when a provider is recycled in place, so it measures provider age rather than registry residency. **A crashed session's END time** is its transcript mtime (`<data home>/sessions/<stem>.jsonl`, appended as the conversation runs, so it is the last moment the session was observably alive and the honest post-hoc maximum). A session with no transcript — a subagent leaves only a replay log — yields no end time and is unlinked WITHOUT an emit rather than recorded as a 0ms lifetime: absence reads as "no data", a recorded zero renders as a real zero. **`_live_starts` is both the normal-path authority and the crumb's sole generation token.** The crumb write goes to the shared maintenance pool (submitted directly rather than through `loop.run_in_executor`, whose loop-bound future raises "Event loop is closed" for a fire-and-forget write that outlives its loop — caught by Windows CI), so the writer can land after its session ended (key popped) or after a successor registered (key holds a newer `started_at`). `_write_crumb` re-reads the table through `_is_current_generation` and writes only while its own `started_at` is still installed, refusing both cases with one comparison and no second set to keep in step. The UNLINK, by contrast, runs INLINE inside `_live_lock`: queued on the pool it could land after a successor wrote its crumb and delete a live session's evidence, and `executors.py::shutdown_maintenance_executor` drains with `cancel_futures=True`, so at `close_all` — when that pool is flooded with teardown work — the unlink would be cancelled outright, leaving a cleanly ended session's crumb for the next boot to back-fill as `crashed` and inflating the very population this instrument exists to measure. Bounded throughout: `_MAX_BACKFILL_EMITS` (2000) caps a runaway crumb directory while still unlinking everything it walks, and `_MAX_LIVE_SESSIONS` (4096) drops the oldest half of the table — both cost samples, never memory. Own bucket family `_SESSION_BUCKETS_MS`. |
...
| `kirocrew.approval.decisions` | counter | `decision` (`allow` / `auto_approve` / `deny` — the gate's own bounded action constant), `security_deny` (bool — separates a hard security refusal from a policy-state deny) | `hooks.py::ToolHookResult.__post_init__`, gated on `from_gate`, which only the result factories set, so it fires exactly once per gate consultation. A surface that OVERRIDES the gate constructs a result directly (the chat runner downgrading an auto-approve to an interactive card, for instance) and is deliberately NOT counted: counting every construction would report one request as two decisions and keep a count for a verdict that was discarded. Both attributes are bounded by construction — no deny reason, tool name or command reaches the recorder. |
...
Five families, each sized to its instrument's measured range:
...
| `_TOOL_CALL_BUCKETS_MS` | 0.5ms – 1h | `tool.call.duration` |
| `_SESSION_BUCKETS_MS` | 1s – 7d | `session.duration` |
